...
Callisto has a `x` position of `5`, then Ganymede's `x` velocity **changes by +1** (because `5 > 3`) and Callisto's `x`
velocity **changes by -1** (because `3` < `5`). However, if the positions on a given axis are the same, the velocity on
...
kinetic energy is the sum of the absolute values of its velocity coordinates.  Below, each line shows the calculations
